...
export function TournamentApp({ initialState, mode, initialLoadError = null }: TournamentAppProps) {
  const [state, setState] = useState<AppState>(initialState);
  const [activeDay, setActiveDay] = useState<TournamentDayId>('day1');
  const [division, setDivision] = useState<DivisionId>(1);
  const [activeTab, setActiveTab] = useState(mode === 'admin' ? 'setup' : 'bracket');
  ...
  const [localImport, setLocalImport] = useState<AppState | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('以前の端末保存データを読み込めませんでした。');
    }
    if (hydratedState) queueMicrotask(() => setLocalImport(hydratedState));
  }, [isAdmin]);

  const applyState = (change: AppState | ((previous: AppState) => AppState)) => {
    if (!isAdmin) return;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        console.error('Failed to save shared tournament state', error);
        setSaveStatus('error');
        setSyncError('共有データを保存できませんでした。入力内容はこの画面に残っています。');
      }
    }, 700);
    return () => {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const response = await fetch('/api/tournament', { cache: 'no-store', signal: controller.signal });
        if (!response.ok) throw new Error(`load failed: ${response.status}`);
        const result = await response.json() as { state: AppState };
        setState(normalizeAppState(result.state));
        setSyncError(null);
      } catch (error) {
        if (!controller.signal.aborted) setSyncError('最新結果を取得できませんでした。自動的に再試行します。');
      }
    };
  ...
  const resetTournament = () => {
    applyState(createInitialAppState());
    setActiveDay('day1');
    setDivision(1);
    setD2View('A');
    setActiveTab('setup');
    ...
    <main className={`app-shell app-shell--${activeDay}`}>
      <header className="topbar">
        <div className="brand-lockup">
          ...
          <div><p className="eyebrow">{isAdmin ? '管理者画面' : '参加者画面'} · {tournamentContext}</p><h1>{activeTournament.name || '大会名未設定'}</h1></div>
        </div>
        <div className="topbar-actions">
          <span className="save-state">
            {isAdmin ? <Save size={14} /> : <RefreshCw size={14} />}
      ...
      <div className="page-wrap">
        {syncError ? <div className="sync-banner sync-banner--error"><span>{syncError}</span>{!isAdmin ? <button type="button" onClick={() => window.location.reload()}>今すぐ更新</button> : null}</div> : null}
        {isAdmin && localImport ? (
          <div className="sync-banner">
            <span>この端末に以前の大会データがあります。共有データへ移すと、参加者画面にも同じ内容が表示されます。</span>
            <button type="button" onClick={importLocalData}><UploadCloud size={16} />共有データへ移行</button>
          </div>
        ) : null}
        <TournamentSelector
          value={activeDay}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="workspace">
          <TabsList className={`step-nav ${isAdmin ? '' : 'step-nav--viewer'}`} aria-label={isAdmin ? '大会管理メニュー' : '大会結果メニュー'}>
          ...
          {isAdmin ? <TabsContent value="setup" className="panel">
