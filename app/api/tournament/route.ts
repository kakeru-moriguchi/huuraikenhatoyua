...
import { getAdminSessionToken } from '@/app/admin-session';
import { isVercelRuntime, vercelAdminEmail } from '@/lib/runtime';
import { getVercelTournamentSnapshot, saveVercelTournament, VercelBridgeError } from '@/lib/vercel-bridge';
...
export async function PUT(request: Request) {
  if (isVercelRuntime()) return handleVercelSave(request);
...
async function handleVercelSave(request: Request) {
  try {
    const email = vercelAdminEmail();
    if (!email) return Response.json({ error: '管理者情報が設定されていません。' }, { status: 503 });
    const body = await request.json() as { state?: unknown };
    const snapshot = await saveVercelTournament(body.state, await getAdminSessionToken(), email);
    return Response.json(snapshot, { headers: { 'cache-control': 'no-store' } });
  } catch (error) {
    console.error('Failed to save Vercel tournament state', error);
